...
use crate::utility::neuronal_layer::neuronal_layer;
...
pub(crate) fn test() {
    ...
    /*for i in 0..2 {
        for j in 0..2 {
            let shouldReturnTrue = (i == 1 && j == 0) || (i == 0 && j == 1); //|| (i == 0 && j == 1);// || (i == 0 && j == 1);// j == 0);// || (i == 0 && j == 1);//i == 1;//j == 1 && i == 0;//(i == 1 && j == 0) || (i == 0 && j == 1);
            println!("shouldReturnTrue: {}, i: {}, j: {}", shouldReturnTrue, i, j);
            let something = if shouldReturnTrue { 1f32 } else { 0.1f32 };
    ...
    network
        ...
        .add_training_data(TrainingData::new(vec![0.0, 1.0], vec![1.0]));
